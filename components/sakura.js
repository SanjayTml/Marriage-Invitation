...
export var Sakura = function Sakura(selector, options) {
  ...
  var el = document.querySelector(selector); // Defaults for the option object, which gets extended below.
  ...
  var settings = extend(defaults, options); // Hide horizontal scrollbars on the target element.

  el.style.overflowX = 'hidden'; // Random array element
  ...
  var createPetal = function () {
    ...
    var fallTime = (document.documentElement.clientHeight * 0.007 + Math.round(Math.random() * 5)) * settings.fallSpeed; // Create animations
    ...
    petal.classList.add(settings.className);
    var height = randomInt(settings.minSize, settings.maxSize);
    var width = height - Math.floor(randomInt(0, settings.minSize) / 3); // Get a random color.

    var color = randomArrayElem(settings.colors);
    ...
    petal.style.borderRadius = "".concat(randomInt(settings.maxSize, settings.maxSize + Math.floor(Math.random() * 10)), "px ").concat(randomInt(1, Math.floor(width / 4)), "px");
  ...
  el.setAttribute('data-sakura-anim-id', window.requestAnimationFrame(createPetal));
};

Sakura.prototype.start = function () {
  var animId = el.dataset.sakuraAnimId;
...
Sakura.prototype.stop = function () {
  ...
  if (animId) {
    ...
    el.setAttribute('data-sakura-anim-id', '');
  ...
  if (!graceful) {
    setTimeout(function () {
      var petals = document.getElementsByClassName(settings.className);
